...
#define HISTORY_CHARACTERS_PER_ENTRY 1500
...
stack* replaceStack(stack*b, stack*e, stack*implant, stack*list){
        return appendStacks(subStack(list,b,list),
        			appendStacks(implant,subStack(e,NULL,list)));
}
...
atom* apply(atom* f, atom* x){
        ...
        if( f->k==FUN && n == f->p.f.arity && (result = f->p.f.f(list)) )
        	return result;
...
atom* fix_fct(stack*list) {
        ...
        atom* _(stack*s){
             ...
             if(verbose) printf("magic: %s\n",magic);
                        char *trick = 
                        	(char*) malloc((strlen(magic)+10)*sizeof(char));
                        strcpy(trick, "\\dummy-> ");
                        strcat(trick, magic);
             ...
             if(verbose){printf("image: ");println(image);}
                        s->head->p.r.cache = 
                        	push(dryApply(x,image),s->head->p.r.cache);
                        return arg(image);
        }
...
char* show(atom* a, bool spaces){
        ...
        switch(a->k){
                ...
                case STRING: 
                        ...
                        free(sub);
                        break;
                case FLOAT:sprintf(result,"%f",a->p.x);break;
...
atom* predicate_fct(stack*list){
        if(!list) return fromString(__func__);
        atom*cond = list->head;
        atom*thn = list->rest->head;
        atom*els = list->rest->rest->head;
        atom*x = list->rest->rest->rest->head;
        if(apply(cond,x)->k == APP) return NULL;
        
        atom* result = apply(cond,x)->p.b ? apply(thn,x) : apply(els,x);
        empty(list);
        return result;
}
        ...
        switch(a1->k){
                case INT:
                        switch(a2->k){
                                ...
                                case FLOAT:
                                        result->k = FLOAT;
                                        result->p.x = (float)a1->p.n + a2->p.x;
                                        break;
                                case CHAR:
                                        result->k = CHAR;
               ...
               case CHAR:
                ...
                case FLOAT:
                	switch(a2->k){
                		case INT:
                                        result->k = FLOAT;
                                        result->p.x = a1->p.x + (float)a2->p.n;
                                        break;
                		case FLOAT:
                                        result->k = FLOAT;
                                        result->p.x = a1->p.x + a2->p.x;
                                        break;
                	}
                	break;
                ...
                case INT:
                        switch(a2->k){
                                ...
                                case CHAR:
                                 ...
                                 case FLOAT:
                                        result->k = FLOAT;
                                        result->p.x = a2->p.x - (float)a1->p.n;
                                        break;
                        }
                        break;
               case CHAR:
                ...
                case FLOAT:
                	switch(a2->k){
                		case INT:
                                        result->k = FLOAT;
                                        result->p.x = (float)a2->p.n - a1->p.x;
                                        break;
                		case FLOAT:
                                        result->k = FLOAT;
                                        result->p.x = a2->p.x - a1->p.x;
                                        break;
                	}
                	break;
                default: result = undefined;
        ...
        switch(a1->k){
        	case INT:
        		switch(a2->k){
        			case INT:
        				result->k = INT;
        				result->p.n = (int)pow((double)a1->p.n, 
        							(double)a2->p.n);
        				break;
        			case FLOAT:
        				result->k = FLOAT;
        				result->p.x = (float)pow((double)a1->p.n,
        							 (double)a2->p.x);
        				break;
        		}
        		break;
        	case FLOAT:
        		switch(a2->k){
        			case INT:
					result->k = FLOAT;
					result->p.x = (float)pow((double)a1->p.x, 
								(double)a2->p.n);
					break;
        			case FLOAT:
        				result->k = FLOAT;
...
atom* euclid_fct(stack*list){
        if(!list) return fromString(__func__);
        atom* set = list->head;

	atom* result = apply2(nth,set,intc(3))->p.n == 0 ? 
		set : 
		apply(euclid,apply(algo_euclid,set));
        empty(list);
        return result;
}
...
atom* _40_60_41_fct(stack*list){
        ...
        if(!x || !y) return false;
        if(x->k == APP || y->k == APP) return NULL;
        ...
        switch(x->k){
        	case INT:
        		result = (y->k == INT ? 
        			x->p.n < y->p.n : 
        			(float) x->p.n < y->p.x) ? true : false;
        		break;
        	case FLOAT:
        		result = (y->k == INT ? 
        			x->p.x < (float) y->p.n : 
        			x->p.x < y->p.x) ? true : false;
        		break;
        	default: result = undefined;
        }
...
atom* abs_fct(stack*list){
        if(!list) return fromString(__func__);
        atom* x = list->head;
        atom* result;
        switch(x->k){
        	case INT: result = intc(abs(x->p.n)); break;
        	case FLOAT: result = floatc(fabs(x->p.x)); break;
        	default: result = undefined;
        }
        empty(list);
        return result;
...
atom* head_fct(stack*list){ // head xs = xs undefined K
        if(!list) return fromString(__func__);
        atom* result;
        if(list->head == undefined) result = undefined;
        else if(list->head->k == CMBNTR_K) result = undefined;
...
atom* dropWhile_fct(stack*list){
        ...
        atom* result = apply(isEmpty,xs)->p.b ? 
                ...
                (!apply(f,apply(head,xs))->p.b ? 
                        xs : 
                        apply2(dropWhile,f, apply(tail,xs)));
        empty(list);
        return result;
}
...
atom* groupBy_fct(stack*list){
	if(!list) return fromString(__func__);
	atom* refl = list->head;
	atom* xs = list->rest->head;
	
	atom* result = apply2(map,snd,
		apply3(foldr,apply(customs,refl),nil,apply2(zip,xs,xs)));
        
        empty(list);
        return result;
}

atom* select_fct(stack* list){
	if(!list) return fromString(__func__);
	int among = list->head->p.n;
	int which = list->rest->head->p.n;
	
	char* str = (char*) malloc((18+4*among) * sizeof(char));
	char var[8]; // from 1 to 99 vars
	strcpy(str,"\\ s -> s (\\");
	for(int i=0; i<among; i++){
		sprintf(var, "a%d ",i+1);
		strcat(str,var);
	}
	sprintf(var,"-> a%d)",which);
	strcat(str,var);
	 atom* result = parse(str);
...
void insertTree(atom*comp, atom*leaf, atom*tree){

	if(!tree->p.r._){
		tree->p.r._ = leaf;
		tree->p.r.arg = new();
		tree->p.r.arg->p.r._ = NULL;
		tree->p.r.arg->p.r.arg = NULL;
		tree->p.r.arg->p.r.iter = NULL;
		tree->p.r.iter = new();
		tree->p.r.iter->p.r._ = NULL;
		tree->p.r.iter->p.r.arg = NULL;
		tree->p.r.iter->p.r.iter = NULL;
	}
	else if(apply2(comp,leaf,tree->p.r._)->p.b) 
		return insertTree(comp,leaf,tree->p.r.iter);
	else return insertTree(comp,leaf,tree->p.r.arg);
}

atom* depthTraverse(atom* tree, atom* acc){
	if(tree->p.r.arg) acc = depthTraverse(tree->p.r.arg,acc);
	if(tree->p.r.iter) acc = depthTraverse(tree->p.r.iter,
						apply2(cons,tree->p.r._,acc));
	return acc;
}

atom* sortBy_fct(stack*list){
	if(!list) return fromString(__func__);
	atom*comp = list->head;
	atom*xs = list->rest->head;
	...
	tree->p.r.iter = NULL;
	while(xs->k != CMBNTR_K){
		insertTree(comp, apply(head,xs), tree);
		xs = apply(tail,xs);
	}
	
	atom* result = depthTraverse(tree,nil);
...
atom* randint_fct(stack*list){
        ...
        atom* max = list->head;
        empty(list);
        atom* result = intc(rand() % max->p.n); // no shortcut    !!
        return result;                          // with this line !!
}
...
atom* toNum(atom* a){
        ...
        if(a->k == CMBNTR_K) return intc(sign*n);
        ...
        if(apply(head,a)->p.c ==  '.'){
        	...
        	while(a->k != CMBNTR_K && isdigit(apply(head,a)->p.c)){
        		tenthpow /= 10.0f;
        		d += (float)(apply(head,a)->p.c - '0') * tenthpow;
        		a = apply(tail,a);
        	}
        	return floatc((float)sign*(d+(float)n));
        }
        return NULL;
}
...
atom* translate(atom*str){
        ...
        else if(isQuoteString(apply(head,str)))
        {
                ...
                while(fun && !areSameStrings(str, fun->head->p.a.f)) 
                	fun = fun->rest;
                if(!fun) {printf("new symbol: "); print(str); printf("\n");}
...
stack* unsugarLets(stack* list, stack*context, bool soft){        
        ...
        if(components){
                ...
                char* abstraction = 
                	contextualize(aggregate(pos->rest,end), newcontext);
                ...
                if(!context){
                        while(expressions){
                                ...
                                subst =  interpret(
                                	parseParenthesis(toString(expressions->head)),
                                	newcontext,0);
...
stack* unAbstracted(stack* components, stack*context){
        ...
        stack*pos = components;
        stack*beg;
        while(pos && (pos->head->k != CHAR || !isbslash(pos->head->p.c))) 
        	pos=pos->rest;
...
void initialize(){ // !!ORDER MATTERS!!
	...
	false = &_false;
        ...
        dividedBy=new();     declareFunction(&dividedBy,   2,  _40_47_41_fct);
        ...
        dropWhile=new();     declareFunction(&dropWhile,   2,  dropWhile_fct);
        ...
        sortBy=new();        declareFunction(&sortBy,      2,  sortBy_fct);
        ...
        power=new();         declareFunction(&power,       2,  _40_94_41_fct);
        module=new();        declareFunction(&module,      1,  abs_fct);
        ...
        groupBy=new();       declareFunction(&groupBy,     2,  groupBy_fct);
        nth=new();           declareFunction(&nth,         2,  _40_33_33_41_fct);
        select_ace=new();    declareFunction(&select_ace,  2,  select_fct);
        ...
        euclid=new();        declareFunction(&euclid,      1,  euclid_fct);
        predicate=new();     declareFunction(&predicate,   4,  predicate_fct);
        ...
        stamp(undefined, fromString("undefined"));
        ...
        min = parse("\\a b -> if a < b then a else b");                      stamp(min, fromString("min"));
        max = parse("\\a b -> if a > b then a else b");                      stamp(max, fromString("max"));
        as = parse("\\ style -> (foldr (:) style) . (flip ap id (zipWith const))");stamp(as, fromString("as"));
        powerSet = parse(
        	"fix (\\ pow xs -> if null xs then [[]] "
        	"                    else let pts = pow (tail xs) "
        	"                           in pts ++ (map ((head xs) :) pts))");stamp(powerSet, fromString("powerSet"));
        cartesianProduct = parse(
        	"fix (\\ cp xs -> if null (tail xs) "
        	"           then concat (map (map (:[])) xs) "
        	"           else let cpt = cp (tail xs) "
        	"         in concat (map (\\ x -> map (x:) cpt) (head xs)))");stamp(cartesianProduct, fromString("cartesianProduct"));
        gcd = parse("\\ x y -> let e = euclid [(max x y),1,0,(min x y),0,1] in head e");stamp(gcd, fromString("gcd"));
        range = parse(" \\ beg f cst iter -> scanl f beg $ take (subtract 1 iter) $ repeat cst");stamp(range, fromString("range"));
        lte = parse("\\ x y -> (x < y) || (x == y)");                         stamp(lte, fromString("(<=)"));
        gte = parse("\\ x y -> (x > y) || (x == y)");                         stamp(gte, fromString("(>=)"));
        on = parse("\\ f g x y -> f (g x) (g y)");                            stamp(on, fromString("on"));
        ...
        group = apply(groupBy,equal);                                stamp(group, fromString("group"));
        sort = apply(sortBy,lessThan);                               stamp(sort, fromString("sort"));
        nub = apply2(B,apply(map,head),apply2(B,group,sort));        stamp(nub, fromString("nub"));
	...
	// for permutations        
        ...
        // for groupBy
        customs = parse (
		"\\refl zt zs -> let z = fst zt"
		"                    t = snd zt"
		"          in if null zs then [[z,[t]]]"
		"                        else let xy = head zs"
		"                               in if refl (fst xy) z"
		"                                   then [fst xy, t: (snd xy)] : (tail zs)"
		"                                   else [z, [t]] : zs");
	// for euclid
	algo_euclid = parse(
		"(\\ set -> let m = set!!0  u = set!!1  v = set!!2 "
...
void tests(){
        ...
        char let1[] = 
                "let test = fix (\\ test n -> if n<0 then [] else n:(test $ n-1))\n"
                "       in as [] $ test 13";
      ...
      char ack1[] = 
        ...
        char fix4[] = 
                "let oddEven = fix (\\ oe n -> if n == 0 then [false, true] \n"
                "               else  map not $ oe (n-1)) \n"
                "               in oddEven 21";
        ...
        char prog1[] = 
                "let trans = (\\ set -> let m = set!!0  u = set!!1  v = set!!2 \n"
                "                                   n = set!!3 ku = set!!4 kv = set!!5 \n"
                "                       in let r = rem m n  d = quot m n \n"
                "                                in  [n,       ku ,      kv, \n"
                "                                     r, u-(d*ku), v-(d*kv)]) \n"
